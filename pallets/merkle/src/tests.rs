...
fn should_have_correct_root_hash_after_insertion() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(MerkleGroups::add_members(Origin::signed(2), 0, vec![
			key1.clone()
		]));
		...
		assert_ok!(MerkleGroups::add_members(Origin::signed(3), 0, vec![
			key2.clone()
		]));
...
fn should_not_verify_invalid_commitments_for_leaf_creation() {
	new_test_ext().execute_with(|| {
		...
		let (_, nullifier_com, leaf_com1, leaf_var1) = commit_leaf(
			&mut test_rng,
			&mut prover,
			leaf,
			s,
...
fn should_verify_zk_proof_of_membership() {
	new_test_ext().execute_with(|| {
		...
		let (s_com, nullifier_com, leaf_com5, leaf_var5) = commit_leaf(
			&mut test_rng,
			&mut prover,
			leaf5,
			s,
