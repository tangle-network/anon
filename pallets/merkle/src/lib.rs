...
pub mod pallet {
	...
	pub enum Event<T: Config> {
		...
		NewMembers(T::GroupId, T::AccountId, u32, u32),
	}
...
impl<T: Config> Group<T::AccountId, T::BlockNumber, T::GroupId> for Pallet<T> {
	...
	fn add_members(
		...
		let num_points_before = tree.leaf_count;
		let num_points = members.len() as u32;
		ensure!(
			num_points_before + num_points <= tree.max_leaves,
		...
		Self::deposit_event(Event::NewMembers(id, sender, num_points_before, num_points));
		Ok(())
	}
