...
// 		assert_ok!(MerkleGroups::create_group(Origin::signed(1), false, Some(1),));
// 		assert_ok!(MerkleGroups::add_members(Origin::signed(1), 0,
// vec![Data(leaf)])); 		let root = MerkleGroups::get_merkle_root(0).unwrap();
...
// 		let comms: Vec<Commitment> = comms_cr.iter().map(|x|
// Commitment(*x)).collect(); 		let leaf_index_comms: Vec<Commitment> =
// leaf_index_comms_cr.iter().map(|x| Commitment(*x)).collect();
// 		let proof_comms: Vec<Commitment> = proof_comms_cr.iter().map(|x|
// Commitment(*x)).collect(); 		assert_ok!(MerkleGroups::
// verify_zk_membership_proof( 			0,
// 			0,
// 			root,
// 			comms,
// 			Data(nullifier_hash),
...
// 		let mut comms: Vec<Commitment> = comms_cr.iter().map(|x|
// Commitment(*x)).collect(); 		let mut rng = OsRng::default();
// 		comms[0] = Commitment(RistrettoPoint::random(&mut rng).compress());
// 		let leaf_index_comms: Vec<Commitment> = leaf_index_comms_cr.iter().map(|x|
// Commitment(*x)).collect(); 		let proof_comms: Vec<Commitment> =
// proof_comms_cr.iter().map(|x| Commitment(*x)).collect(); 		assert_err!(
...
// 		let comms: Vec<Commitment> = comms_cr.iter().map(|x|
// Commitment(*x)).collect(); 		let mut leaf_index_comms: Vec<Commitment> =
// leaf_index_comms_cr.iter().map(|x| Commitment(*x)).collect();
// 		let mut proof_comms: Vec<Commitment> = proof_comms_cr.iter().map(|x|
// Commitment(*x)).collect(); 		let mut rng = OsRng::default();
// 		leaf_index_comms[0] = Commitment(RistrettoPoint::random(&mut
// rng).compress()); 		proof_comms[0] = Commitment(RistrettoPoint::random(&mut
// rng).compress()); 		assert_err!(
// 			MerkleGroups::verify_zk_membership_proof(
// 				0,
// 				0,
// 				root,
...
// leaf_index_comms_cr.iter().map(|x| Commitment(*x)).collect();
// 		let proof_comms: Vec<Commitment> = proof_comms_cr.iter().map(|x|
// Commitment(*x)).collect(); 		assert_err!(
// 			MerkleGroups::verify_zk_membership_proof(
// 				0,
// 				0,
// 				root,
...
// 		let keys_data: Vec<Data> = keys.iter().map(|x|
// Data(Scalar::from_bytes_mod_order(*x))).collect(); 		assert_ok!(MerkleGroups::
// create_group(Origin::signed(1), false, Some(3),)); 		assert_ok!(MerkleGroups::
// add_members(Origin::signed(1), 0, keys_data));

// 		let root = MerkleGroups::get_merkle_root(0).unwrap();
// 		let (proof, (comms_cr, nullifier_hash, leaf_index_comms_cr, proof_comms_cr))
// = 			ftree.prove_zk(root.0, leaf5, &ftree.hash_params.bp_gens, prover);
...
// 		let mut ftree =
// FixedDepositTreeBuilder::new().hash_params(poseidon).depth(32).build();
