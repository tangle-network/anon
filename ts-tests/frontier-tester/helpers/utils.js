...
const BINARY_PATH = `../../target/release/webb-node`;
...
async function startAnonNode() {
  ...
  const web3 = await new Promise((resolve) => {
    const errHandle = () => {
      console.error(`\x1b[31m Failed to start Anon Node.\x1b[0m`);
...
function describeWithAnon(title, cb) {
  describe(title, async () => {
    const context = { web3: null, binary: null };
    before("Starting Anon Node", async () => {
      const data = await startAnonNode();
      context.binary = data.binary;
      context.web3 = data.web3;
    });

    after('Exiting Anon Node', async () => {
