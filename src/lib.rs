...
impl<T: Trait> MTree<T> {
	pub fn new(root: String, fee: T::Balance, depth: u32) -> Self {
		MTree {
			root: root,
			fee: fee,
			depth: depth,
...
decl_storage! {
	trait Store for Module<T: Trait> as SparseMerkleTree {
		...
		pub MerkleTreeLevels get(merkle_tree_level): map (u32, u32) => Option<Vec<String>>;
...
decl_module! {
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		pub fn create_new_tree(origin, root: Option<String>, fee: Option<T::Balance>, depth: Option<u32>, leaves: Option<Vec<String>>) -> Result {
			let _sender = ensure_signed(origin)?;
			let should_update = match root { Some(_) => false, None => false };
			let root = match root { Some(r) => r, None => "".to_string() };
		...
		pub fn add_leaf(origin, tree_id: u32, leaf: String) -> Result {
			let _sender = ensure_signed(origin)?;
...
impl<T: Trait> Module<T> {
	...
	pub fn add_batch_leaves(tree_id: u32, leaf_elements: Vec<String>) {
	...
	pub fn add_leaf_element(tree_id: u32, leaf: String) {
		let mut tree = <MerkleTreeMetadata<T>>::get(tree_id).ok_or("Tree doesn't exist").unwrap();
		let level_key = (tree_id, 0);
		if let Some(mut leaf_level) = <MerkleTreeLevels>::get(level_key) {
			leaf_level.push(leaf);
	...
	pub fn get_unique_node(leaf: String, _index: usize) -> String {
		if leaf != "".to_string() {
			return leaf;
		} else {
			return "".to_string();
	...
	fn mimc(left: String, right: String, rounds: u32) -> String {
		let k = T::Field::from_str("21888242871839275222246405745257275088548364400416034343698204186575808495617").unwrap();
		let mut l = T::Field::from_str(&left).unwrap();
		let mut r = T::Field::from_str(&right).unwrap();
